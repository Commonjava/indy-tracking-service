...
import org.commonjava.indy.service.tracking.model.dto.TransferDTO;
...
public class AdminController
{
    ...
    private Set<TransferDTO> constructTransferDTOSet( final Set<TrackedContentEntry> entries )
    {
        if ( entries == null )
        {
            return null;
        }
        Set<TransferDTO> cut_entries = new HashSet<>();
        for ( TrackedContentEntry entry : entries )
        {
            TransferDTO cut_entry =
                            new TransferDTO( entry.getStoreKey(), entry.getTrackingKey(), entry.getAccessChannel(),
    ...
    {
        Set<TransferDTO> transfer_entries = constructTransferDTOSet( entries );
